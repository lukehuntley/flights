...
{% set get_start_date_query %}
    select min(to_char(TO_DATE(TO_CHAR(flightdate),'YYYYDDMM'),'YYYY-MM-DD')) from {{ ref('stg_flights') }}
{% endset %}
{%- set start_date = dbt_utils.get_single_value(get_start_date_query) -%}

{% set get_end_date_query %}
    select max(to_char(TO_DATE(TO_CHAR(flightdate),'YYYYDDMM'),'YYYY-MM-DD')) from {{ ref('stg_flights') }}
{% endset %}
